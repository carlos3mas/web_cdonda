...
export function SponsorsBanner() {
  ...
  return (
    <div className="fixed top-16 left-0 right-0 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-200 overflow-hidden py-1.5 sm:py-2 shadow-sm">
      <div className="relative">
        ...
        >
          {duplicatedSponsors.map((logo, index) => (
            <div
              key={`${logo}-${index}`}
              className="flex-shrink-0 flex items-center h-10 sm:h-12 md:h-14 w-28 sm:w-32 md:w-40 opacity-70 hover:opacity-100 transition-opacity px-1.5 sm:px-2"
            >
